...
const DetailedView = ({
  ...
  const stationSelected = useSelector(
    (state: Pick<StateMapToPropsGlobal, 'currentFaultsPage'>) =>
      state.currentFaultsPage.stationActive
  );
  const data: Array<React.ReactElement>[] = [[]];
  const idChildren: Array<string>[] = [[]];
  const failList: Array<typeof stationSelected.fail_list> = [[]];
  const payload: CurrentFaultsPayload[] = message
    ? message?.filter(
        (station: { fail_list: string | any[] }) => station.fail_list.length > 0
      )
    : [];
  ...
  useEffect(() => {
    const filteredDispatch: CurrentFaultsPayload[] = (
      payload as CurrentFaultsPayload[]
    ).filter((payload) => payload.label === stationSelected.label);
    if (
      filteredDispatch.length &&
      filteredDispatch[0].fail_list.length !== stationSelected.fail_list.length
    ) {
                                ...
                                <S.FailItem key={`${fail.name}_${idx}`}>
                                  ...
                                  <div className="fail-item__quantity">
                                    ...
                                    <Tooltip
                                      description={
                                        <span>
                                          Quantidade acumulada de eventos de
                                          falha do tipo:{' '}
