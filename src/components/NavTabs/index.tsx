...
import { KeysOfPagesContainingTimer, SettingsSwitchTabsProps, StateMapToPropsGlobal, SwitchChartView } from 'types';
...
const NavTabs: React.FC<
  Types.NavTabsChartProps<React.ReactNode> &
    Types.MainProps &
    Pick<SettingsSwitchTabsProps, 'actionAutomaticMode' | 'namespace' | 'actionTimer'>
> = ({
  children,
  isHeightFull,
  data,
  ...
  return (
    <S.Container namespace={namespace as KeysOfPagesContainingTimer}>
      {!loader && (
        <ContentLoader viewBox="0 0 1862 915" animate backgroundColor="#EFF3F8" foregroundColor="#D8DDE6">
          <path
      ...
      {loader && (
        <>
          ...
          <S.Main heightScreen={settingsGlobal.heightNavigation} isHeightFull={isHeightFull} padding={padding}>
            {!data.length
              ? ''
              : data.map((component, idx) => (
                  <Animation.BottomToTop key={idx} idItem={idx} classes={namespace as string} active={active}>
